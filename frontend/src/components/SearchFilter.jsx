    ...
    <div className="relative">
      <button
        ...
        className={`px-5 w-56 h-full border border-black rounded-sm transition-all duration-150 ease-in-out ${
          isHidden ? "bg-white hover:bg-black hover:text-white" : "bg-black text-white "
        }`}
      >
        {value}
      </button>
      <div
        className={`absolute flex flex-col gap-2 w-60 shadow-md rounded-b-xl p-3 mt-2
            transform transition-all duration-100 origin-top ${
              isHidden ? "opacity-0 scale-y-0 pointer-events-none" : "opacity-100 scale-y-100"
            }`}
      >
        {options.map((option, index) => (
          <button
            ...
            } hover:bg-gray-100 transition-all rounded-md duration-100`}
          >
            {option.text}
